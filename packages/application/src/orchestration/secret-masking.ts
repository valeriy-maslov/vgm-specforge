...
export function sanitizeRunForPersistence(run: WorkflowRun): WorkflowRun {
  return maskSensitiveData(run);
}
...
function maskValue(value: unknown, seen: WeakSet<object>): unknown {
  ...
  for (const [key, entry] of Object.entries(inputRecord)) {
    if (isSensitiveKey(key)) {
      outputRecord[key] = entry === undefined ? undefined : REDACTED;
      continue;
    }

    outputRecord[key] = maskValue(entry, seen);
  }
